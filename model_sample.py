...
class TextGenerator:
    def __init__(
        ...
        dtype="bfloat16",
    ):  # 数据类型，默认为 float32，可以选择 float16 或 bfloat16
        """
        ...
        self.model = Transformer(
            ModelConfig(dim=1024, n_layers=18)
        )  # 实例化 Transformer 模型
    ...
    def sft_sample(
        ...
        top_k=300,
    ):  # 保留概率最高的 top_k 个 token，限制生成时的选择范围
        """
        根据给定的起始文本生成样本。
            ...
            with self.ctx:  # 进入自动混合精度的上下文（如果是 GPU 并使用 float16 时）
                for k in range(num_samples):  # 循环生成指定数量的样本
                    ...
                    generated_texts.append(
                        self.tokenizer.decode(y[0].tolist())
                    )  # 解码生成的 token 序列为可读文本
...
if __name__ == "__main__":
    ...
    sft_prompt_datas = ["你好呀", "中国的首都是哪里？", "1+12等于多少？", "你是谁？"]
    generator = TextGenerator(
